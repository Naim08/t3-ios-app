...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Call the database function to spend tokens atomically
    const dbResponse = await fetch(`${supabaseUrl}/rest/v1/rpc/spend_user_tokens`, {
      method: 'POST',
      headers: {
    ...
    if (!dbResponse.ok) {
      ...
      if (errorText.includes('insufficient_credits')) {
        return new Response(
